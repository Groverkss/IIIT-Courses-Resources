# Operating Systems and Networks Resources

## Remzi, OSTEP

**Resource Name**: Remzi Spring'18 and OSTEP

**Contributor (your) Name**: Shashwat Goel

**Resource Links**: http://pages.cs.wisc.edu/~remzi/Classes/537/Spring2018/

**Have you completed studying this resource?**: Lectures yes, book and homework no.

**Your review of this resource**: The lectures are great for an intuitive understanding

**Why do you think this resource will be useful for others?**: OSN course at IIIT uses a boring graduate textbook with a lot of detail. Since the teaching is poor, achieving clarity of concepts or finding the coursework interesting is almost impossible without learning elsewhere. Hence, using the intuitive lectures here and the conceptual easy-to-read book (OSTEP) can be useful to most.

##

## Introduction to Computer Networking | Stanford

**Resource Name**: Introduction To Computer Networking

**Contributor (your) Name**: Kunwar Shaanjeet Singh Grover

**Resource Links**: 
1. Videos: https://www.bilibili.com/video/BV137411Z7LR ~~(The original youtube links got deleted. Since everything on internet is permanent, here is a chineese link)~~ Someone has uploaded the playlist from Bilibili onto Youtube as well unofficially https://www.youtube.com/playlist?list=PL6RdenZrxrw9inR-IJv-erlOKRHjymxMN
2. Notes: https://github.com/khanhnamle1994/computer-networking
3. Assignments:  https://github.com/PrincetonUniversity/COS461-Public

**Have you completed studying this resource?**: Completed 6/8 sections

**Your review of this resource**: Video letures are lengthy but really good. The demos using wireshark and shell utilities in between lectures are really helpful in understanding the topics practically. Watching this course alone can be boring. Pick atleast one person along with you to watch as the topics can get hard to grasp at times and can become boring.

**Why do you think this resource will be useful for others?**: IIIT Prof tries to cover a very heavy book in less than a month time and usually is very theoretical with no history, reasons or demos (particularly this). This course is much better than trying to understand inclass lectures. Another thing worth mentioning is that i tried finding various resources to study networking from and found this to be the best one according to me.  

##

## xv6 Resources

**Resource Name**: xv6 Resources

**Contributor (your) Name**: Kunwar Shaanjeet Singh Grover

**Resource Links**: 
1. https://ocw.mit.edu/courses/electrical-engineering-and-computer-science/6-828-operating-system-engineering-fall-2012/lecture-notes-and-readings/MIT6_828F12_xv6-book-rev7.pdf
2. https://github.com/YehudaShapira/xv6-explained

**Have you completed studying this resource?**: Read relevant parts for xv6 assignments

**Your review of this resource**: (1) is the official book with explanations that comes with xv6. (2) Are some great explanations of how and each and every part of it works. Great explanations which make you understand how things in a kernal actually work. For example a thing you can try to find yourself is "how does sleep syscall work in a kernal?"

**Why do you think this resource will be useful for others?**: From what I saw in my batch, most people either copied the xv6 assignments or did not actually understand what and how they were doing it. These resources help you understand and will enhance what you will learn from that assignment or from xv6 in general.

##

## OS course by Mythili Vutukuru (Prof at IIT Bombay)

**Resource Name:** OS course by Mythili Vutukuru  (Prof at IIT Bombay) (lectures and notes) along with content related to XV6

**Contributor (your) Name**: Anmol Agarwal

**Resource Links:**
1. Lecture Notes (2016 version) : https://www.cse.iitb.ac.in/~mythili/teaching/cs347_autumn2016/index.html
2. XV-6 content (2020 version) : https://www.cse.iitb.ac.in/~mythili/os/
3. Lectures: https://youtube.com/playlist?list=PLTtjs-HViBW7HPvOPDNarA-e1WAU23tKG

**Have you completed studying this resource?:** The course seems to be divided into 7 parts: A,B,C,D,X,N,Z. Parts A,B,C,D have accompanying lectures. Some parts reference the **OSTEP Three Easy Pieces** as lecture notes while the other parts have notes compiled by the instructor herself.
I have viewed lectures and read notes of parts A,B,C,D and X. Haven't done the assignments here.

**Your review of this resource:** The lectures have great clarity content wise and the notes (the ones which have been compiled by the instructor herself) are extremely concise,well-written(Eg:https://www.cse.iitb.ac.in/~mythili/teaching/cs347_autumn2016/notes/02-processes-threads.pdf) and the sequence in which topics are covered is perfect.

**Why do you think this resource will be useful for others?:**
The lectures by the IIIT instructor (in Monsoon 2020) were boring and did not add anything to the content already covered by the course textbook(Silberschatz). Prof Vutukuru uses the OSTEP book as the main reference textbook (which helps if you are already planning to use the OSTEP Three Easy Pieces book)  and the **lecture videos are short (~20 min)** but still more insightful and informative than the entire course at IIIT. Even if you don’t have enough time to view the lectures, the written notes (in link 1) are likely to confirm several of your own thoughts, conclusions, insights etc which you derive from the course.

##

Shaanjeet 10/08/2020

**Resource link**: https://www.nand2tetris.org/ 
**Your review of this resource**: This isnt exactly OS but a combination of DSM, Computer Organisation and OS. Why this course instead of the courses we have? This course is a programming heavy course. You actually program every single piece of thing you use or make with the only thing that is premade is a NAND gate. The goal of the course is to have a working computer that can run simple programs like a Tetris Game. Seems like a very good overview of systems and im doing it because im interested in knowing how would you actually build a computer starting from ground up.

##

Anmol Agarwal 15/08/2020

Not a resource per se, but you can try your hand on different distributions of Linux here ( kind of like a simulator)   https://distrotest.net/

##
Shaanjeet 15/08/2020

http://pages.cs.wisc.edu/~remzi/Classes/537/Spring2018/

Course is well structured. Assignments are similar to our OS course (building a shell and stuff) but they cover more topics. The guy teaches really really well and has amazing sense of humour. The book prescribed is eben more amazing.

##
Ishaan Shah 16/08/2020

**Resource link**: https://www.cs.cmu.edu/~guna/15-123S11/Lectures/Lecture24.pdf
**Your review of this resource**: A good resource for learning basics of system calls in C.

##
Aman Rojjha 19/08/2020

[file-implementation-Simple.pdf](/Operating-Systems-and-Networks/file-implementation-Simple.pdf) was shared last year for this assignment. Should've shared earlier (Even I forgot about this!)

##
Shaanjeet 28/08/2020

Every Programmer Should Know About Memory: https://akkadia.org/drepper/cpumemory.pdf
Info related to swap: https://chrisdown.name/2018/01/02/in-defence-of-swap.html

##
Shaanjeet 28/08/2020

**Resource link**: http://www.linuxfromscratch.org/lfs/
**Your review of this resource**: Linux From Scratch (LFS) is a project that provides you with step-by-step instructions for building your own customized Linux system entirely from source.

##
Pulak Malhotra 28/08/2020

Another resource to write your own OS from scratch (University of Birmingham): [Writing_Simple_OS_from_Scratch.pdf](/Operating-Systems-and-Networks/Writing_Simple_OS_from_Scratch.pdf)

##
Shashwat Goel 29/08/2020

https://www.cs.uic.edu/~jbell/CourseNotes/OperatingSystems/ - OS Quick Revision notes by a prof from a course referring the same textbook

##
Shaanjeet 29/08/2020

If you have no clue on how to do the OS assignment, use this --> http://pages.cs.wisc.edu/~remzi/OSTEP/cpu-api.pdf

##
Shashwat Goel 01/09/2020

Some resources on creating command line interfaces

https://azrael.digipen.edu/~mmead/www/Courses/CS180/getopt.html - Using getopt to take arguments
https://eng.localytics.com/exploring-cli-best-practices/ - CLI best practices
https://zapier.com/engineering/how-to-cli/ - CLI best practices 
https://softwareengineering.stackexchange.com/questions/307467/what-are-good-habits-for-designing-command-line-arguments - stylistic stuff on command line arguments 
https://stackoverflow.com/questions/9725675/is-there-a-standard-format-for-command-line-shell-help-text - printing usage messages the right way

##
Rahul Goel 03/09/2020

Helpful for the Assignment. Would have saved me 2 hours if I had watched this: https://www.youtube.com/watch?v=83M5-NPDeWs

##
Dhruv Kapur 04/09/2020

Will help in handling the background process termination part: https://www.oracle.com/technical-resources/articles/it-infrastructure/dev-signal-handlers-studio.html

##
Aman Rojjha 05/09/2020

http://cs.brown.edu/courses/csci1310/2020/

##
Shashwat Goel 06/09/2020

Webpage of the Yale course taken by the author of our book (Silberschatz). Has some useful material like review questions, some QnA, slides etc - https://codex.cs.yale.edu/avi/os-book/OS10/index.html

##
Zeeshan 08/09/2020

Worth skimming through once for The /proc Filesystem: https://www.qnx.com/developers/docs/6.5.0SP1.update/com.qnx.doc.neutrino_cookbook/s3_procfs.html

##
Shashwat Goel 09/09/2020

https://www.gnu.org/software/libc/manual/html_node/Concepts-of-Job-Control.html
https://www.gnu.org/software/libc/manual/html_node/Launching-Jobs.html
https://www.gnu.org/software/libc/manual/html_node/Process-Group-Functions.html


##
Shashwat Goel 11/09/2020

What every programmer should know about memory: https://lwn.net/Articles/250967/

##
Rahul Goel 10/10/2020

Code-first thread introductory videos.
https://youtu.be/uA8X5zNOGw8
https://youtu.be/It0OFCbbTJE
https://www.youtube.com/watch?v=9axu8CUvOKY

##
Anmol Agarwal 11/10/2020

Excellent resource to understand implementation of monitors (elaborates on the partial explanation given in the book)
https://cseweb.ucsd.edu/classes/sp17/cse120-a/applications/ln/lecture8.html

##
Shaanjeet 21/10/2020

Lecture Slides for Philip Levis and Nick McKeown's "Introduction to Computer Networking" Stanford course: https://github.com/khanhnamle1994/computer-networking

##
Shashwat Goel 23/10/2020

* https://pdos.csail.mit.edu/6.828/2020/schedule.html - MIT's 2020 OS course with all materials completely available online. Taught by Robert Morris, others. Will definitely be useful for the xv6 assignment as the whole course is xv6 based.
* https://pdos.csail.mit.edu/6.828/2020/xv6/book-riscv-rev1.pdf  - The accompanying 'book' (just 110 pages) seems quite amazing too. The shell we banged our heads with for so long, all the knowledge reqd is explained succintly in chapter 1 itself. Shell is just the week1 lab assignment in the course:)
**NOTE** (by Shaanjeet): *This is apparently the newer version of the book. xv6-public is x86 architecture and no longer maintained (ofcourse os tas give us unmaintained shit). The book you mention is for the riscv architecture. Just a keep in mind for anyone planning to read that some things may be different. If you want to check out the maintained version of xv6, https://github.com/mit-pdos/xv6-riscv.*

##
Shaanjeet 24/10/2020

https://pdos.csail.mit.edu/6.828/2018/xv6/book-rev11.pdf - Th accompanying book for xv6-x86 architecture

##
Zeeshan 04/11/2020

Amazing explanation of the internet protocol
https://www.youtube.com/watch?v=rPoalUa4m8E&list=PLowKtXNTBypH19whXTVoG3oKSuOcw_XeW&index=8

##
Shaanjeet 04/11/2020

https://www.cs.dartmouth.edu/~campbell/cs50/socketprogramming.html
For socket programming. Useful resource. If you want to finish withen 1-2h just use the code from here. Good explanations for everything too.

##
Shashwat Goel 05/11/2020

Seems like slides were taken from here. Video lectures by the book co-author himself. Only issue is can't speed up and he speaks too slow
https://media.pearsoncmg.com/ph/streaming/esm/tanenbaum5e_videonotes/tanenbaum_videoNotes.html
*(Note by Aman: To speed up the lectures Shashwat sent, use document.getElementsByTagName("video")[0].playbackRate = 2 in the console.)*
